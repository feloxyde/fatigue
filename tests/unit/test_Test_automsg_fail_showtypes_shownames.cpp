...
struct TestAutoMsgFail : public Test {
    ...
    virtual void run(){
        ...
        res << "(2) [ERROR] expected check_near_equal( l: 20 [int], r: 12 [int], tolerance: 2 [int] ) to succeed, but failed." << std::endl;
        assert(ss.str() == res.str());
        ...
        res << "(3) [ERROR] expected check_near_equal( l: 7 [int], r: 10 [int], tolerance: 2 [int] ) to succeed, but failed." << std::endl;
        assert(ss.str() == res.str());
